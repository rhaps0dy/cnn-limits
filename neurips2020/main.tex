...
independence and mean-pooling\adriacomment{mean-pooling (which is equivalent to complete
  correlation in the weights)?}. Our approach allows the discrete architectural choice of mean-pooling to be replaced with a more flexible continuous amount of correlation. Empirical evaluation on CIFAR-10 shows that this additional flexibility improves performance.

Our work illustrates that there are choices \adriacomment{about the weight
  prior} to be made when analysing infinite limits of neural networks, and that good choices can improve performance. The success of this approach in the infinite limit also raises questions about whether correlated weights should be used in finite networks.
...
\begin{figure}
    \scalebox{1.0}{\input{fig/cntk14.pgf}}
  \caption{CNTK14}
\end{figure}
