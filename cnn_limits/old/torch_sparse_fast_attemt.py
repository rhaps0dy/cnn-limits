...
def patch_kernel_fn(kernel_fn, strides, W_covs, forego_mean_pooling):
    ...
    assert isinstance(W_covs, list)
    W_cov_diagonals_list = []
    for W_cov in W_covs:
        ...
        W_cov_diagonals_list.append(W_cov_diagonals)
    if len(W_cov_diagonals_list) > 0:
        W_cov_diagonals_list = np.asarray(onp.stack(W_cov_diagonals_list, axis=2))
    else:
        W_cov_diagonals_list = None
    ...
    def _patch_kernel_fn(z1, start_idx1, mask1, z2, start_idx2, mask2, get='nngp'):
        ...
        if isinstance(outputs, list):
            assert all(out.is_height_width for out in outputs)
            assert stride == 1
            _var_mask = outputs[0].var_mask

            outputs = [out.nngp for out in outputs]
            if ntk is not None:
                outputs = outputs + [out.ntk for out in outputs]
        ...
        out_list = []
        if not forego_mean_pooling:
            meanp_outputs = outputs * _var_mask
            *_, H, W = outputs.shape
            meanp_outputs = meanp_outputs.sum((-2, -1)) * (1/(H*H*W*W))
            out_list.append(meanp_outputs)


        if W_cov_diagonals_list is not None:
            assert sz1 == sz2, "not implemented"
            _out_ij_1, _out_ij_2 = (ij_1, ij_2)
            idx = np.transpose(_out_ij_1[:, :, 1:], (1, 0, 2)) - _out_ij_2[:, :, 1:] + (sz1-1)
            W = np.squeeze(lax.gather(
                W_cov_diagonals_list, idx, lax.GatherDimensionNumbers(
                    offset_dims=(2, 3, 4, 5, 6),
                    collapsed_slice_dims=(),
                    start_index_map=(0, 1)),
                slice_sizes=(1, 1, *W_cov_diagonals_list.shape[2:])), axis=(2, 3))

            W_idx = 'ji'
            if stacked:
                out_list.append(np.einsum(f"dabij,abc{W_idx}->dcab", outputs, W).reshape((-1, height, width)))
            else:
                out_list.append(np.einsum(f"abij,abc{W_idx}->ab", outputs, W))
