...
def main_no_eig(kernel_matrix_path, _log, n_splits, i_SU):
    ...
    with h5py.File(kernel_matrix_path/"kernels.h5", "r") as f:
        ...
        data = pd.DataFrame(index=range(1, N_layers, 2), columns=all_N)
        data_max = pd.DataFrame(index=range(1, N_layers, 2), columns=all_N)
        accuracy = pd.DataFrame(index=range(1, N_layers, 2), columns=all_N)
        ...
        for layer in layer_iter:
            ...
            Kxx = f['Kxx'][layer-1].astype(dtype)
            Kxx[mask] = Kxx.T[mask]
            if not np.allclose(Kxx, Kxx.T):
                print("Skiping layer {layer} for nans")
                continue
            ...
            Kxt += f['Kxt'][layer-1].astype(dtype)
